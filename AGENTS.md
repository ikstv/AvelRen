...
Новий файл `db/migrations/NNN_опис.sql`, послідовний номер. Застосовує їх
сервіс `migrate` — але **тільки коли його явно попросили**:

```bash
docker compose --profile migrate up migrate
```

Він за профілем `migrate` і не входить у дефолтний набір, тож `compose up`
його не тягне і схема сама собою не змінюється. Це навмисно: застосовувач
накатує всі невідомі йому `*.sql` без піна максимальної версії, тож неявний
підйом означав би застосування міграції, до якої стек ще не готовий (саме так
010 могла заїхати повз adoption). Ordering не втрачено: коли профіль
увімкнено, `collector`/`api` і далі чекають завершення `migrate`.
